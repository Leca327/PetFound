...
public class ContPetCadastrar extends javax.swing.JFrame {

    private Connection connection;
    Boolean maxc;
    String adm, audio, tipocont, nome;

    public ContPetCadastrar(String admin, String au) {
        initComponents();
        adm = admin;
        audio = au;
        setIcon();
    ...
    public static String generateContpetCode(String name, String pet) {
        LocalDateTime now = LocalDateTime.now();

        // Remover os espaços das strings nick e name
        name = name.replaceAll("\\s", "");
        pet = pet.replaceAll("\\s", "");
    ...
    public void buspet() {
        ...
        try {
            ...
            // Limpar itens existentes do ComboBox
            while (res.next()) {
                comb_pet.removeItem("Sem Versões Guardadas");
            ...
            try {
                ...
                if (rs.next()) {
                    ...
                    if (tx1.equals(textf_pf.getText())) {
                        ...
                        ps = connection.prepareStatement("select * from contatopet where pfcodp=? and petcodpet=?;");
                        ps.setString(1, user);
                        ps.setString(2, pet);
                        rs = ps.executeQuery();
                        if (rs.next()) {
                            alert al = new alert(adm, audio);
                            al.setVisible(true);
                            String msg = "Contato para este Pet";
                            ...
                            if (!rs.next()) {
                                ...
                                if ("PJ".equals(user.substring(0, 2))) {
                                    ...
                                    String msg1 = "Criar um contato para pet";
                                    String tit = "Pessoa existente";
                                    al.alertinput(tit, "erro", msg, msg1, "", "erro");
                                    ...
                                    if (!rs.next()) {
                                        ...
                                        if ((textf_pf.getText().isEmpty()) || (textf_pet.getText().isEmpty())) {
                                            ...
                                            ContPetMOD cp = new ContPetMOD();
                                            String date = DtAtual();
                                            String hr = HrAtual();
                                            cp.setTpcont(tipocont);
                                            cp.setCodcontp(contpetcod);
                                            cp.setPetcod(textf_pet.getText());
                                            cp.setPfcod(textf_pf.getText());
                                            cp.setDt(date);
                                            cp.setHr(hr);

                                            // instanciando a classe UsuarioDAO do pacote dao e criando seu objeto dao
                                            ContPetDAO dao = new ContPetDAO();
                                            dao.adiciona(cp);
                                            alert al = new alert(adm, audio);
                                            al.setVisible(true);
                                            String msg = "Contato do Pet " + nome;
                                            String msg2 = " inserido(a) com sucesso";
                                            String tit = "Cadastro de Contato Pet";
    ...
    private void initComponents() {
        ...
        setTitle("Pet Found - Cadastrar Contato Pet");
        setResizable(false);
        ...
        textf_pe.setToolTipText("Nome do Pet que está sendo escolhido");
        ...
        btn_cadastrar.setToolTipText("Cadastre Um Contato Pet");
        ...
        if (!userInput.isEmpty()) {
            ...
            try {
                ...
                if (res.next()) {
                    ...
                    textf_pf1.setText("Pet Não Encontrada");
                }
    ...
    public static void main(String args[]) {
        java.awt.EventQueue.invokeLater(new Runnable() {
            public void run() {
                String au = "";
                String ad = "";
                new ContPetCadastrar(ad, au).setVisible(true);
            }
        });
    }
