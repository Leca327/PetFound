...
import subGUI.PFisicaAtualizar;
import subGUI.PFisicaCadastrar;

public class PFisicaBuscar extends javax.swing.JInternalFrame {
    ...
    public PFisicaBuscar(String admin, String au, TelaPrin telaPrin) {
        initComponents();
        ...
        jTable1.addMouseListener(new java.awt.event.MouseAdapter() {
            ...
            public void mouseClicked(java.awt.event.MouseEvent evt) {
                ...
                seluser = jTable1.getValueAt(row, 4).toString();
                selcod = jTable1.getValueAt(row, 7).toString();
                setImg(seluser);
                busdst(seluser);
    ...
    public void bus(String user) {
        ...
        if (adm != null || adm != "") {
            if (permchefe == true) {
                ...
                btn_vsenha.setToolTipText("Ver Senha de Pessoa Física");
            } else {
                btn_vsenha.setEnabled(false);
                btn_vsenha.setToolTipText("Sem Permissão para Ver Senha de Pessoa Física");
            }
        }
    }
    ...
    public void limfiltro() {
        cb_dst.setSelectedItem("Todas Pessoas Física At/Dst");
        cb_ordem.setSelectedItem("Mais Recentes");
        cb_sx.setSelectedItem("Todos os Sexos");
        cb_idade.setSelectedItem("Todas as Idades");
    ...
    public void bloqbus() {
        ...
        textf_buscar.setToolTipText("Selecione a A forma que deseja Pesquisar Pessoa Física.");
    }
        ...
        switch (tipoveri) {
            case 1: {
                ...
                try {
                    switch (tipo2) {
                        case 1:
                            ps = connection.prepareStatement("select * from fisica where codp=?;");
                            break;
                        case 3:
                            ps = connection.prepareStatement("SELECT * FROM pessoa JOIN fisica ON fisica.codp = pessoa.pcod JOIN endereco ON endereco.endcod = pessoa.endcodend where emailp=?;");
                            break;
                        case 4:
                            ps = connection.prepareStatement("SELECT * FROM pessoa JOIN fisica ON fisica.codp = pessoa.pcod JOIN endereco ON endereco.endcod = pessoa.endcodend where contatop=?;");
                            break;
                        default:
                            ps = connection.prepareStatement("SELECT * FROM pessoa JOIN fisica ON fisica.codp = pessoa.pcod JOIN endereco ON endereco.endcod = pessoa.endcodend where nickname=?;");
                            break;
                    }
                    ps.setString(1, cod);
        ...
        if (dst == false) {
            ...
            try {
                ...
                if (i != 0) {
                    alert al = new alert(adm, audio);
                    al.setVisible(true);
                    String msg = "Detalhes da Pessoa Física";
    ...
    public void setImg(String user) {
        this.connection = new ConnectionFactory().getConnection();
        String sql = "SELECT * FROM pessoa JOIN fisica ON fisica.codp = pessoa.pcod JOIN endereco ON endereco.endcod = pessoa.endcodend where nickname=?";
        try {
            PreparedStatement stm = connection.prepareStatement(sql);
            stm.setString(1, user);
            ResultSet res = stm.executeQuery();
    ...
    public void buscarpf(String user) {
        resul = 0;
        ...
        switch (selectedOption) {
            case "Pessoas Físicas Desativadas":
                bloq = " AND pessoa.bloqueiop=true";
                break;
            case "Pessoas Físicas Ativas":
        ...
        selectedOption = (String) cb_sx.getSelectedItem();
        if ("Todos os Sexos".equals(selectedOption)) {
            sx = "";
        } else {
            sx = " and sexo='" + selectedOption + "'";
        }

        String idade = "";
        selectedOption = (String) cb_idade.getSelectedItem();
        if (null != selectedOption) {
            switch (selectedOption) {
                case "Todas as idades":
                    idade = " AND YEAR(CURDATE()) - YEAR(fisica.dt_nascimento) > 17";
                    break;
                case "18 a 24 anos":
                    idade = " AND YEAR(CURDATE()) - YEAR(fisica.dt_nascimento) BETWEEN 18 AND 24";
                    break;
                case "25 a 29 anos":
                    idade = " AND YEAR(CURDATE()) - YEAR(fisica.dt_nascimento) BETWEEN 25 AND 29";
                    break;
                case "30 a 34 anos":
                    idade = " AND YEAR(CURDATE()) - YEAR(fisica.dt_nascimento) BETWEEN 30 AND 34";
                    break;
                case "35 a 45 anos":
                    idade = " AND YEAR(CURDATE()) - YEAR(fisica.dt_nascimento) BETWEEN 35 AND 45";
                    break;
                case "46 a 60 anos":
                    idade = " AND YEAR(CURDATE()) - YEAR(fisica.dt_nascimento) BETWEEN 46 AND 60";
                    break;
                case "Mais de 60 anos":
                    idade = " AND YEAR(CURDATE()) - YEAR(fisica.dt_nascimento) > 60";
                    break;
                default:
                    break;
            }
        ...
        switch (tipo2) {
            case 1: {
                this.connection = new ConnectionFactory().getConnection();
                String sql = "";
                sql = "SELECT * FROM pessoa JOIN fisica ON fisica.codp = pessoa.pcod JOIN endereco ON endereco.endcod = pessoa.endcodend where codp=?" + bloq + idade + sx + ord;
                try {
                    PreparedStatement stm = connection.prepareStatement(sql);
                    ...
                    Object[][] tabelpf = new Object[1][18]; // Número de colunas é 10

                    while (res.next()) {
                        resul++;
                        tabelpf[0][0] = res.getString(1);
                        tabelpf[0][1] = res.getString(11);
                        tabelpf[0][2] = res.getString(2);
                        tabelpf[0][3] = res.getString(3);
                        tabelpf[0][4] = res.getString(5);
                        tabelpf[0][5] = res.getString("dt_nascimento");
                        tabelpf[0][6] = res.getString("sexo");
                        tabelpf[0][7] = res.getString(7);
                        tabelpf[0][8] = res.getString(9);
                        tabelpf[0][9] = res.getString(10);
                        tabelpf[0][10] = res.getString(4);
                        tabelpf[0][11] = res.getString("cep");
                        tabelpf[0][12] = res.getString("uf");
                        tabelpf[0][13] = res.getString("cidade");
                        tabelpf[0][14] = res.getString("bairro");
                        tabelpf[0][15] = res.getString("endereco");
                        tabelpf[0][16] = res.getString("numero");
                        tabelpf[0][17] = res.getString("cmpt");

                    }

                    // Define os nomes das colunas
                    Object[] columnNames = {"Nome", "Sobre Nome", "Contato", "Email", "Nick", "Data de Nascimento", "Sexo", "Código", "Data de Criação", "Hora de Criação", "Cód Endereço", "CEP", "UF", "Cidade", "Bairro", "Endereço", "Número", "Complemento"};

                    // Cria um modelo de tabela personalizado usando os dados e nomes das colunas
                    CustomTableModel model = new CustomTableModel(tabelpf, columnNames);

                    // Configura o modelo da tabela
                    jTable1.setModel(model);
                    sql = "SELECT * FROM pessoa JOIN fisica ON pessoa.pcod = fisica.codp where codp=?" + bloq + idade + sx + ord;
            ...
            case 3: {
                this.connection = new ConnectionFactory().getConnection();
                String sql = "";
                sql = "SELECT * FROM pessoa JOIN fisica ON fisica.codp = pessoa.pcod JOIN endereco ON endereco.endcod = pessoa.endcodend where emailp=?" + bloq + idade + sx + ord;
                try {
                    PreparedStatement stm = connection.prepareStatement(sql);
                    stm.setString(1, user); // Define o valor do parâmetro
                    ResultSet res = stm.executeQuery();
            ...
            case 4: {
                this.connection = new ConnectionFactory().getConnection();
                String sql = "";
                sql = "SELECT * FROM pessoa JOIN fisica ON fisica.codp = pessoa.pcod JOIN endereco ON endereco.endcod = pessoa.endcodend where contatop=?" + bloq + idade + sx + ord;
                try {
                    PreparedStatement stm = connection.prepareStatement(sql);
                    stm.setString(1, user); // Define o valor do parâmetro
                    ResultSet res = stm.executeQuery();
            ...
            case 5: {
                ...
                this.connection = new ConnectionFactory().getConnection();
                String sql = "";
                sql = "SELECT * FROM pessoa JOIN fisica ON fisica.codp = pessoa.pcod JOIN endereco ON endereco.endcod = pessoa.endcodend where nickname=?" + bloq + idade + sx + ord;
                try {
                    PreparedStatement stm = connection.prepareStatement(sql);
                    stm.setString(1, user); // Define o valor do parâmetro
                    ResultSet res = stm.executeQuery();
        ...
        if ("Pessoas Físicas Desativadas".equals(selectedOption)) {
            btn_dst.setIcon(new ImageIcon(PFisicaBuscar.class.getResource("/img/icon_atv.png")));
        } else {
            btn_dst.setIcon(new ImageIcon(PFisicaBuscar.class.getResource("/img/icon_dest.png")));
        }
    ...
    public void BPersPf(String user) {
        resul = 0;
        ...
        switch (tipo2) {
            case 1: {
                this.connection = new ConnectionFactory().getConnection();
                String sql = "";
                sql = "SELECT * FROM pessoa JOIN fisica ON fisica.codp = pessoa.pcod JOIN endereco ON endereco.endcod = pessoa.endcodend where fisica.codp like ?" + bloq + idade + sx + ord;
                ...
                try {
                    ...
                    if (result) {
                        while (result) {
                            ...
                            row[1] = res.getString(11);
                            row[2] = res.getString(2);
                            row[3] = res.getString(3);
                            row[4] = res.getString(5);
                            row[5] = res.getString("dt_nascimento");
                            row[6] = res.getString("sexo");
            ...
            case 5: {
                this.connection = new ConnectionFactory().getConnection();
                String sql = "";
                sql = "SELECT * FROM pessoa JOIN fisica ON fisica.codp = pessoa.pcod JOIN endereco ON endereco.endcod = pessoa.endcodend where pessoa.nickname like ?" + bloq + idade + sx + ord;
            ...
            case 4: {
                this.connection = new ConnectionFactory().getConnection();
                String sql = "";
                sql = "SELECT * FROM pessoa JOIN fisica ON fisica.codp = pessoa.pcod JOIN endereco ON endereco.endcod = pessoa.endcodend where pessoa.contatop like ?" + bloq + idade + sx + ord;
            ...
            case 3: {
                this.connection = new ConnectionFactory().getConnection();
                String sql = "";
                sql = "SELECT * FROM pessoa JOIN fisica ON fisica.codp = pessoa.pcod JOIN endereco ON endereco.endcod = pessoa.endcodend where pessoa.emailp like ?" + bloq + idade + sx + ord;
    ...
    public void buscarTodospf() {
        ...
        String idade = "";
        selectedOption = (String) cb_idade.getSelectedItem();
        if (" WHERE".equals(bloq) && sx.equals("")) {
            if (null != selectedOption) {
                switch (selectedOption) {
                    case "Todas as idades":
                        idade = " YEAR(CURDATE()) - YEAR(fisica.dt_nascimento) > 17";
                        break;
                    case "18 a 24 anos":
                        idade = " YEAR(CURDATE()) - YEAR(fisica.dt_nascimento) BETWEEN 18 AND 24";
                        break;
                    case "25 a 29 anos":
                        idade = " YEAR(CURDATE()) - YEAR(fisica.dt_nascimento) BETWEEN 25 AND 29";
                        break;
                    case "30 a 34 anos":
                        idade = " YEAR(CURDATE()) - YEAR(fisica.dt_nascimento) BETWEEN 30 AND 34";
                        break;
                    case "35 a 45 anos":
                        idade = " YEAR(CURDATE()) - YEAR(fisica.dt_nascimento) BETWEEN 35 AND 45";
                        break;
                    case "46 a 60 anos":
                        idade = " YEAR(CURDATE()) - YEAR(fisica.dt_nascimento) BETWEEN 46 AND 60";
                        break;
                    case "Mais de 60 anos":
                        idade = " YEAR(CURDATE()) - YEAR(fisica.dt_nascimento) > 60";
                        break;
                    default:
                        break;
                }
        ...
        if (" WHERE".equals(bloq) && "".equals(sx) && "".equals(idade)) {
            bloq = "";
        }

        this.connection = new ConnectionFactory().getConnection();
        String sql = "SELECT * FROM pessoa JOIN fisica ON fisica.codp = pessoa.pcod JOIN endereco ON endereco.endcod = pessoa.endcodend " + bloq + sx + idade + ord;
    ...
    public void delet(String cod) {
        ...
        try {
            ...
            // Excluir contatoserv relacionados à pessoa física
            ps = connection.prepareStatement("DELETE FROM contatoserv WHERE pcodp IN (SELECT pcod FROM pessoa WHERE pcod = ?);");
            ps.setString(1, cod);
            int i = ps.executeUpdate();

            // Excluir contatopet relacionados à pessoa física
            ps = connection.prepareStatement("DELETE FROM contatopet WHERE pfcodp IN (SELECT codp FROM fisica WHERE codp = ?);");
            ps.setString(1, cod);
            i = ps.executeUpdate();

            // Excluir imagem relacionada à pessoa física
            ps = connection.prepareStatement("DELETE FROM imagem WHERE petcodpet IN (SELECT petcod FROM pet WHERE pessoacodp = ?);");
            ps.setString(1, cod);
            i = ps.executeUpdate();

            // Excluir imagem relacionada à pessoa física
            ps = connection.prepareStatement("DELETE FROM imagem WHERE servicocodserv IN (SELECT servcod FROM servico WHERE pessoa_codp = ?);");
            ps.setString(1, cod);
            i = ps.executeUpdate();

            // Excluir pets relacionados à pessoa física
            ps = connection.prepareStatement("DELETE FROM pet WHERE pessoacodp = ?;");
            ps.setString(1, cod);
            i = ps.executeUpdate();

            // Excluir serviços relacionados à pessoa física
            ps = connection.prepareStatement("DELETE FROM servico WHERE pessoa_codp = ?;");
            ps.setString(1, cod);
            i = ps.executeUpdate();
            ...
            // Excluir pessoa física e tabela fisica
            ps = connection.prepareStatement("DELETE pessoa, fisica FROM pessoa INNER JOIN fisica ON pessoa.pcod = fisica.codp WHERE pessoa.pcod = ?;");
            ps.setString(1, cod);
            i = ps.executeUpdate();
            ...
            if (i != 0) {
                ...
                alert al = new alert(adm, audio);
                al.setVisible(true);
                String msg = "Pessoa Física " + seluser;
                String msg2 = "deletado do database";
                String tit = "Deleção de Pessoa Física";
                al.alertinput(tit, "ok", msg, msg2, "", "sucesso");
                buscarTodospf();
            } else {
                ...
                alert al = new alert(adm, audio);
                al.setVisible(true);
                String msg = "Pessoa Física inexistente ";
                String tit = "Deleção de Pessoa Física";
                al.alertinput(tit, "erro", "", msg, "", "erro");
            }
        } catch (SQLException e) {
    ...
    private void initComponents() {
        ...
        cb_idade = new javax.swing.JComboBox<>();
        cb_dst = new javax.swing.JComboBox<>();
        cb_sx = new javax.swing.JComboBox<>();
        ...
        btn_cad.setToolTipText("Cadastrar Pessoa Física");
        ...
        textf_result.setToolTipText("Resultados Encontrados na Tabela de Pessoa Física\n");
        ...
        Cb_pf.setModel(new javax.swing.DefaultComboBoxModel<>(new String[] { "Código Pessoa Física", "Nick", "E-mail", "Contato" }));
        ...
        textf_buscar.setToolTipText("Selecione a forma que deseja Pesquisar a Pessoa Física");
        ...
        jLabel1.setText("Buscar Pessoa Física");
        ...
        cb_ordem.setToolTipText("Pessoa Física em Ordem de data Crescente/Decrescente");
        cb_ordem.setCursor(new java.awt.Cursor(java.awt.Cursor.HAND_CURSOR));

        cb_idade.setBackground(new java.awt.Color(255, 253, 243));
        cb_idade.setModel(new javax.swing.DefaultComboBoxModel<>(new String[] { "Todas as Idades", "18 a 24 anos", "25 a 29 anos", "30 a 34 anos", "35 a 45 anos", "46 a 60 anos", "Mais de 60 anos" }));
        cb_idade.setToolTipText("Faixa-Etária de Pessoa Física");
        cb_idade.setCursor(new java.awt.Cursor(java.awt.Cursor.HAND_CURSOR));
        cb_idade.setMinimumSize(new java.awt.Dimension(202, 22));
        cb_idade.setPreferredSize(new java.awt.Dimension(202, 22));

        cb_dst.setBackground(new java.awt.Color(255, 253, 243));
        cb_dst.setModel(new javax.swing.DefaultComboBoxModel<>(new String[] { "Todas Pessoas Física At/Dst", "Pessoas Físicas Ativas", "Pessoas Físicas Desativadas" }));
        cb_dst.setToolTipText("Pessoa Física Ativas(At)/Desativadas(Dst)");
        ...
        cb_sx.setBackground(new java.awt.Color(255, 253, 243));
        cb_sx.setModel(new javax.swing.DefaultComboBoxModel<>(new String[] { "Todos os Sexos", "M", "F" }));
        cb_sx.setToolTipText("Sexo de Pessoa Física");
        cb_sx.setCursor(new java.awt.Cursor(java.awt.Cursor.HAND_CURSOR));
        cb_sx.setMinimumSize(new java.awt.Dimension(202, 22));
        cb_sx.setPreferredSize(new java.awt.Dimension(202, 22));
        ...
        jTable1.setModel(new javax.swing.table.DefaultTableModel(
            ...
            new String [] {
                "Nome", "Sobre Nome", "Contato", "Email", "Nick", "Data de Nascimento", "Sexo", "Código", "Data de Criaçao", "Hora de Criação", "Cód Endereço", "CEP", "UF", "Cidade", "Bairro", "Endereço", "Número", "Complemento"
            }
        ) {
        ...
        pass_versenha.setToolTipText("Senha da Pessoa Física Selecionada");
        ...
        btn_pet.setText("Ver Pet da Pessoa Física");
        ...
        btn_pet1.setText("Ver Serviço da Pessoa Física");
            ...
            .addGroup(PFundoLayout.createSequentialGroup()
                ...
                .addGroup(PFundoLayout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
                        ...
                        .addGroup(PFundoLayout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
                            .addGroup(PFundoLayout.createSequentialGroup()
                                .addGroup(PFundoLayout.createParallelGroup(javax.swing.GroupLayout.Alignment.TRAILING)
                                    .addComponent(label_selecionado, javax.swing.GroupLayout.Alignment.LEADING)
                                    .addGroup(javax.swing.GroupLayout.Alignment.LEADING, PFundoLayout.createSequentialGroup()
                                        .addComponent(cb_idade, javax.swing.GroupLayout.PREFERRED_SIZE, javax.swing.GroupLayout.DEFAULT_SIZE, javax.swing.GroupLayout.PREFERRED_SIZE)
                                        .addGap(224, 224, 224)
                                    ...
                                    .addGroup(javax.swing.GroupLayout.Alignment.LEADING, PFundoLayout.createSequentialGroup()
                                        ...
                                        .addPreferredGap(javax.swing.LayoutStyle.ComponentPlacement.RELATED)
                                        .addComponent(cb_sx, javax.swing.GroupLayout.PREFERRED_SIZE, javax.swing.GroupLayout.DEFAULT_SIZE, javax.swing.GroupLayout.PREFERRED_SIZE))
                                    .addGroup(javax.swing.GroupLayout.Alignment.LEADING, PFundoLayout.createSequentialGroup()
                                    ...
                                    .addComponent(label_vsenha, javax.swing.GroupLayout.Alignment.LEADING))
                    ...
                    .addGroup(javax.swing.GroupLayout.Alignment.TRAILING, PFundoLayout.createSequentialGroup()
                        ...
                        .addPreferredGap(javax.swing.LayoutStyle.ComponentPlacement.RELATED)
                        .addComponent(btn_pet1, javax.swing.GroupLayout.PREFERRED_SIZE, 177, javax.swing.GroupLayout.PREFERRED_SIZE)
            ...
            .addGroup(javax.swing.GroupLayout.Alignment.TRAILING, PFundoLayout.createSequentialGroup()
                ...
                .addGroup(PFundoLayout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING, false)
                    ...
                    .addGroup(PFundoLayout.createSequentialGroup()
                        ...
                        .addGroup(PFundoLayout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
                            .addComponent(cb_idade, javax.swing.GroupLayout.PREFERRED_SIZE, javax.swing.GroupLayout.DEFAULT_SIZE, javax.swing.GroupLayout.PREFERRED_SIZE)
                            .addGroup(javax.swing.GroupLayout.Alignment.TRAILING, PFundoLayout.createParallelGroup(javax.swing.GroupLayout.Alignment.BASELINE)
    ...
    public void versenha() {
        ...
        if (cont2 == 0) {
            ...
            cont2 = 1;
            btn_vsenha
                    .setIcon(new ImageIcon(PFisicaBuscar.class
    ...
    public void dst() {
        ...
        if (adm == null || adm.equals("")) {
            ...
            if (seluser == null) {
                ...
                String tit = "Nenhuma Pessoa Física selecionado";
                al.alertinput(tit, "info", msg, msg2, "", "info");
                ...
                if (dst == false) {
                    audios("aviso");
                    int escolha = JOptionPane.showConfirmDialog(null, "Deseja Mesmo Ativar a Pessoa Física " + seluser + "?");
                    ...
                    audios("aviso");
                    int escolha = JOptionPane.showConfirmDialog(null, "Deseja Mesmo Desativar a Pessoa Física " + seluser + "?");
    ...
    public void cad() {
        ...
        if (adm == null || adm.equals("")) {
            ...
            audios("tc");
            PFisicaCadastrar pf = new PFisicaCadastrar(adm, audio);
            pf.setVisible(true);
        }
    }
                ...
                if ("null".equals(adm) || !"null".equals(selcod)) {
                    if (adm == null || adm.equals("")) {
                        ...
                        if (selcod == null) {
                            ...
                            String tit = "Nenhum Pet selecionado";
                            al.alertinput(tit, "info", msg, msg2, "", "info");

                        } else {
                            audios("tc");
                            PFisicaAtualizar ad = new PFisicaAtualizar(adm, audio, seluser);
                            ad.setVisible(true);
                        }
                    }
                }
    ...
    private void Cb_pfActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_Cb_pfActionPerformed
        if (Cb_pf.getSelectedItem() != null) {
            ...
            textf_buscar.setEditable(false);
            textf_buscar.setToolTipText("Selecione a A forma que deseja Pesquisar a Pessoa Física.");
            Lbuscar.setCursor(Cursor.getPredefinedCursor(Cursor.DEFAULT_CURSOR));
    ...
    private void pass_versenhaActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_pass_versenhaActionPerformed
        ...
        if ("Mais Recentes".equals(selectedOption)) {
            cb_ordem.setToolTipText("Buscar Pessoa Física Mais Recentes");
        } else {
            cb_ordem.setToolTipText("Buscar Pessoa Física Mais Antigos");
        }
    ...
    private TelaPrin getTelaPrinInstance() {
        // Implemente a lógica para obter a instância da TelaPrin, dependendo da sua estrutura
        // Supondo que você tenha uma referência válida para a TelaPrin
        return telaPrin;
    }
    ...
    private javax.swing.JComboBox<String> cb_idade;
    private javax.swing.JComboBox<String> cb_ordem;
    private javax.swing.JComboBox<String> cb_sx;
