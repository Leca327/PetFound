...
if (isset($_GET['ordenacao'])) {
    if ($_GET['ordenacao'] == "mais_novos") {
        $ordenacao = " ORDER BY dtp DESC, hrp DESC";
    } elseif ($_GET['ordenacao'] == "mais_antigos") {
        $ordenacao = " ORDER BY dtp ASC, hrp ASC";
    }
}
...
if ($aprovacaoFilter == "todos") {
    ...
    if (isset($_GET['dsta'])) {
        if ($_GET['dsta'] == "atv") {
            $dsta = " WHERE (bloqueiopet = false OR bloqueiopet IS NULL)";
        } elseif ($_GET['dsta'] == "dst") {
            $dsta = " WHERE bloqueiopet = true";
        }
    }
} else {
    $dsta = "";
    if (isset($_GET['dsta'])) {
        if ($_GET['dsta'] == "atv") {
            $dsta = " AND (bloqueiopet = false OR bloqueiopet IS NULL)";
        } elseif ($_GET['dsta'] == "dst") {
            $dsta = " AND bloqueiopet = true";
        } elseif ($_GET['dsta'] == "td") {
            $dsta = "";
        }
    }
...
if (isset($_GET['aprovados'])) {
    ...
    $countConsulta = "SELECT COUNT(*) AS qtd FROM pet";
    if ($_GET['aprovados'] == "aprovado") {
        $countConsulta .= " WHERE aprovacaopet = true";
    } else if ($_GET['aprovados'] == "nao_analisado") {
        $countConsulta .= " WHERE aprovacaopet IS NULL";
    } else if ($_GET['aprovados'] == "reprovado") {
        $countConsulta .= " WHERE aprovacaopet = false";
    }
} else {
    $countConsulta = "SELECT COUNT(*) AS qtd FROM pet";
}
...
<body>
    ...
    <form method="get" action="">
        ...
        <div class="filtro">
            <label class="aprovado" for="aprovados">Aprovação:</label>
            ...
            <label class="dsta" for="dsta">Pets ativos/desativados:</label>
    ...
    <form method="POST" action="updatepet.php">
        ...
        <table class="tbladm" border="1">
            <h5 class="poss"><?php echo $qtdpet; ?> Pets Encontrados</h5>

            <tr>
                <td></td>
                <td class="CA">Código do Pet</td>
                <td class="CA">Código do Anunciante</td>
                <td class="TA">Tipo de Anuncio</td>
                <td class="NP">Nome do pet</td>
                <td class="TP">Tipo de Pet</td>
                <td class="FE">Faixa-Etária</td>
                <td class="S">Sexo</td>
                <td class="R">Raça</td>
                <td class="P">Porte</td>
                <td class="CPL">Cor do Pelo</td>
                <td class="E">Estado/UF</td>
                <td class="C">Cidade</td>
                <td class="AP">Aprovação do Pet</td>
                <td class="APA">Aprovado pelo ADM</td>
                <td class="REP">Motivo de Reprovar</td>
                <td class="DR">Data de registro</td>
                <td class="HR">Hora de registro</td>
            </tr>
            <?php while ($dado = $con->fetch_array()) {
                ...
                $final = $dado["finalidade"];
                switch ($dado["finalidade"]) {
                    case "Adocao":
                        $final = "Adoção";
                        break;
                    case "Pad_Ado":
                        $final = "Padrinho e Adoção";
                        break;
                }

                $bloq = $dado["bloqueiopet"];
                switch ($dado["bloqueiopet"]) {
                    case 1:
                        $bloq = "Ativar";
                        break;
                    case null:
                ...
                $apv = $dado["aprovacaopet"];
                switch ($dado["aprovacaopet"]) {
                    case 1:
                        $apv = "Aprovado";
            ...
            ?>
                ...
                <tr class="dados">
                    <td><input type="checkbox" name="selected_pets" value="<?php echo $dado["petcod"]; ?>" id="pet_<?php echo $dado["petcod"]; ?>" onclick="checkButtonStatus()"><input type="hidden" name="pet_names" value="<?php echo $dado["nomepet"]; ?>"><input type="hidden" name="pet_aprovacao_<?php echo $dado["petcod"]; ?>"  value="<?php echo $apv; ?>"></td>
                    <td><?php echo $dado["petcod"]; ?></td>
                    <td><?php echo $dado["pessoacodp"]; ?></td>
                    <td><?php echo $final; ?></td>
                    <td><?php echo $dado["nomepet"]; ?></td>
                    <td><?php echo $dado["tipop"]; ?></td>
                    <td><?php echo $dado["fai_ida"]; ?></td>
                    <td><?php echo $sx; ?></td>
                    <td><?php echo $dado["raca"]; ?></td>
                    <td><?php echo $porte; ?></td>
                    <td><?php echo $dado["cor_pel"]; ?></td>
                    <td><?php echo $dado["estadop"]; ?></td>
                    <td><?php echo $dado["cidadep"]; ?></td>
                    <td><?php echo $apv; ?></td>
                    <td><?php echo $dado["admincodadmn"]; ?></td>
                    <td><?php echo $dado["motivoreppet"]; ?></td>
                    <td><?php echo date("d/m/y", strtotime($dado["dtp"])); ?></td>
                    <td><?php echo $dado["hrp"]; ?></td>


                    <td>
                        <a class="editsadm" href="../../anuncio/perfilpet.php?petcod=<?php echo $dado["petcod"]; ?>">Vizualizar </a>
                        <a class="delsadm" href="excluir.php?codigo=<?php echo $dado["petcod"]; ?>&apv=<?php echo $bloq; ?>"><?php echo $bloq; ?></a>
                    </td>
                </tr>
            <?php } ?>
        ...
        <div class="btns">
        <button class="btnS1" type='submit' name='approve_button' disabled="disabled">Aprovar Pet</button>
        <button class="btnS2" type='button' id='reje_button' name='reje_button' disabled="disabled" onclick='toggleReprovarTextarea()'>Reprovar Pet</button>
