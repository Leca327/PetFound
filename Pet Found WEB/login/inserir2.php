...
if ($mysqli) {
    ...
    if (isset($_POST['nome']) && isset($_POST['cep']) && isset($_POST['brr']) && isset($_POST['cpmt']) && isset($_POST['city']) && !empty($_POST['uf']) && isset($_POST['email']) && isset($_POST['senha']) && isset($_POST['celular']) && isset($_POST['usuario']) && isset($_POST['endereco']) && isset($_POST['tipo']) && isset($_POST['cnpj']) && isset($_POST['rma'])) {
        ...
        if (strlen($nome) <= 50 && strlen($usuario) <= 50 && strlen($email) <= 100 && strlen($senha) <= 25 && strlen($rma) <= 50 && strlen($endereco) <= 50 && strlen($brr) <= 25 && strlen($nm) <= 5 && strlen($cpmt) <= 25) {
            $celularf = preg_replace("/[^0-9]/", "", $celular);
            $cnpjf = preg_replace("/[^0-9]/", "", $cnpj);
    ...
    LOWER(nickname) = LOWER('$usuario')";
                ...
                'error'
                    ...
                    if ($total > 0) {
                        ...
                        $query = "SELECT * FROM juridica WHERE cnpj = '$cnpjf'";
                        $result = mysqli_query($mysqli, $query);
                        $total = mysqli_num_rows($result);
                        if ($total > 0) {
                            while ($row = $result->fetch_assoc()) {
                ...
                'error'
                    ...
                    window.history.back();
                            ...
                            $cod = "PJ" . $inicial_nick . $hora_atual . $inicial_nome . $data_atual;
                            $codsub = substr($cod, 0, 6);
                            $codend = "ENDpj" . $inicial_nome . $hora_atual . $codsub . $data_atual;
                            ...
                            $sql = "INSERT INTO pessoa (nickname, emailp, contatop, endcodend, senha,nomep,pcod,dtcriacao,hrcriacao) VALUES ('$usuario', '$email', '$celularf','$codend',  '$senha','$nome','$cod','$data_atual2','$hora_atual2')";
                            $sql2 = "INSERT INTO juridica (cnpj, ramo_ativ, tipoj,cod_p) VALUES ('$cnpjf', '$rma','$tp' ,'$cod')";
